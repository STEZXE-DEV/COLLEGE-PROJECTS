...
def wyswietl_los():
    ...
    komunikat.configure(okno_aplikacji, text=f"Playing sound - recording{los}.mp3...", font=("Helvetica", 30))
    odtworz_dzwiek()
    if komunikat.winfo_exists():  # Sprawdzenie, czy widżet komunikat istnieje
        komunikat.configure(text=f"Playing sound - recording{los}.mp3...", font=("Helvetica", 30))
...
def odtworz_dzwiek():
    def odtwarzanie():
        ...
        if os.path.exists(sciezka_mp3): # Sprawdzenie czy plik audio (MP3) istnieje
            ...
            komunikat.configure(text=f"File not found!", font=("Helvetica", 30)) # Jeżeli plik audio (MP3) nie istnieje, to zostanie wypisany komunikat
...
# Ustawienia etykiet
wynik_losowania = ctk.CTkLabel(okno_aplikacji, text="Click \"Draw!\" to randomly select a recording!", font=("Helvetica", 60))
...
przycisk_losowania = ctk.CTkButton(okno_aplikacji, text="Draw!", command=wyswietl_los, font=("Arial", 40, "bold"), height=100, width=350)
przycisk_losowania.pack(side="top", padx=10, pady=10)

przycisk_zamkniecia = ctk.CTkButton(okno_aplikacji, text="Exit", command=zakoncz_program, fg_color="#eb0e0e", hover_color="#732525", font=("Arial", 40, "bold"), height=100, width=350)
